...
class PyVideoListener : public VideoListener {
public:
    PyVideoListener(std::function<void(const VideoEvent&)> callback, int eventMask, bool immediateNotify) :
        VideoListener(callback, eventMask, immediateNotify),
        m_callback(callback)
    {}

    virtual ~PyVideoListener() {}
...
PYBIND11_MODULE(_cscore, m) {
    ...
    py::class_<PyVideoListener> videolistener(m, "VideoListener");
    videolistener.doc() = 
